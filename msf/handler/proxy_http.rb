...
module ProxyHttp
	...
	#
	def full_uri
		phost = datastore['PHOST']
		if phost.empty? or phost == "0.0.0.0" or phost == "::"
			exit(1) #fix
		end
		#lhost = "[#{lhost}]" if Rex::Socket.is_ipv6?(lhost)
                scheme = (ssl?) ? "https" : "http"
		uri = "#{scheme}://#{phost}:#{datastore["PPORT"]}/"
		...
		register_options(
			[
				...
				OptPort.new('LPORT', [ true, "The local listener port", 8080 ]),
                                OptString.new('PHOST', [ true, "Hostname of proxy" ]),
                                OptPort.new('PPORT', [ true, "Perimeter proxy port", 443 ])
